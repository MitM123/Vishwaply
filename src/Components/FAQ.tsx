          ...
          <div className="flex flex-col-reverse md:flex-row w-full">
            <div className="font-dm-sans w-full md:w-[60%] mx-auto space-y-4 p-4 md:p-7">
              <Accordion type="single" collapsible className="w-full">
                {faqs.map((faq, index) => (
                  <AccordionItem key={index} value={`item-${index}`} className="border-b mb-4 border-indigo-200/50">
                    <AccordionTrigger
                      className="text-left py-4 px-4 md:px-6 bg-blue-100/50 hover:bg-indigo-100/70 rounded-t-lg hover:no-underline transition-colors duration-200"
                    >
                      <span className="text-base md:text-lg font-semibold text-slate-800">{faq.question}</span>
                    </AccordionTrigger>
                    <AccordionContent className="p-4 md:p-6 text-start bg-blue-50/60 rounded-b-lg">
